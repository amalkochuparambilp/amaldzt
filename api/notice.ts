...
export default function handler(req: any, res: any) {
  ...
  if (req instanceof Request || (!res && typeof (req as any)?.headers?.get === 'function')) {
    ...
    try {
      ...
      if (fs.existsSync(noticePath)) {
        ...
        return new Response(JSON.stringify({ success: true, active: Boolean(data?.active), notice: data }), {
          status: 200,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        });
      }
      return new Response(JSON.stringify({ success: true, active: false, notice: null }), {
        status: 200,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    } catch (e: any) {
  ...
  try {
    ...
    if (fs.existsSync(noticePath)) {
      const raw = fs.readFileSync(noticePath, 'utf8');
      const data = JSON.parse(raw);
      return res.status(200).json({ success: true, active: Boolean(data?.active), notice: data });
    }
    return res.status(200).json({ success: true, active: false, notice: null });
  } catch (e: any) {
