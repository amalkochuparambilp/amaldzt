...
export default function MetaRayBanConverter({ onBack }: MetaRayBanConverterProps) {
  ...
  const handleFile = (file: File) => {
    if (!file.type.includes('jpeg') && !file.type.includes('jpg') && !/\.jpe?g$/i.test(file.name)) {
      setStatus({
        message: 'Please select a JPG or JPEG photo.',
        type: 'error'
      });
      return;
    }
      ...
      <div className="bg-[#0e0e0e] border border-white/15 rounded-xs p-6 sm:p-8 space-y-6 relative overflow-hidden shadow-2xl">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs sm:text-sm text-gray-400 max-w-md mx-auto leading-relaxed">
            1-Tap 3024×4032 format scaler, Meta AI EXIF metadata injection, Base64 extraction & direct camera roll save.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <span className="text-xs font-mono font-bold uppercase tracking-wider">Photo Library</span>
          </button>
        </div>
        ...
        >
          <div className="flex flex-col items-center justify-center gap-2">
            ...
            <p className="font-mono font-bold text-sm text-white">
              {sourceFileName || 'Tap or Drop Photo to Convert'}
            </p>
            <p className="text-xs text-white/50 font-mono">
              {sourceFileSize > 0
                ? `${(sourceFileSize / 1024).toFixed(1)} KB • Ready to inject Meta EXIF`
                : 'Supports any JPG / JPEG image'}
            </p>
          </div>
        </div>
        ...
        {sourceDataUrl && (
          <div className="relative rounded-xs overflow-hidden border border-white/15 bg-black">
            ...
            <div className="absolute top-3 right-3 bg-black/80 backdrop-blur-xs border border-white/20 px-2.5 py-1 rounded-xs text-[11px] font-mono text-cyan-400 font-bold">
              {finalDataUrl ? '3024×4032 EXIF Injected' : 'Source Loaded'}
            </div>
